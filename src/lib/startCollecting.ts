...
type Row = Partial<Omit<StartCollectingItem, "price" | "stock">> & {
  id: string;
...
function mapRow(row: Row): StartCollectingItem {
  return {
    id: row.id,
    name: row.name ?? "",
...
// legacy image field.
export function startCollectingImages(item: StartCollectingItem): string[] {
  if (item.images.length > 0) return item.images;
  return item.image ? [item.image] : [];
}

// Shape a Start Collecting item like a shop Product so the product page
// components (gallery / info / specs / cards) can be reused as-is.
export function startCollectingToProduct(item: StartCollectingItem): Product {
  return {
    id: item.id,
    name: item.name,
