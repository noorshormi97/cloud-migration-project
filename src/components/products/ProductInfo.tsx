...
interface ProductInfoProps {
  product: Product;
  // Which cart bucket this item belongs to — shop products by default, but
  // the New Arrivals / Start Collecting pages pass their own kind so the
  // order system charges from the right table.
...
export function ProductInfo({ product, cartKind = "product" }: ProductInfoProps) {
  ...
  const handleAddToCart = () => {
    if (!inStock) return;
    addToCart(product.id, Math.min(quantity, product.stock), cartKind);
  ...
  return (
    <div className="space-y-3.5">
      ...
      <div className="space-y-1.5 font-sans text-sm font-light tracking-wide text-ink/80">
        <p>
          <span className="font-medium text-ink/50">Denomination:</span> {product.denomination}
        </p>
        <p>
          <span className="font-medium text-ink/50">Year:</span> {product.year}
        </p>
        <p>
          <span className="font-medium text-ink/50">Condition:</span> {product.condition}
        </p>
      </div>
